...
Verification intent: Prove external consumers cannot invoke provider mutations, then test every provider, policy, plan, endpoint, route, controller, state, generation, and capacity rejection with fresh dispatch-counted no-mutation snapshots.
...
Verification intent: Exercise valid and invalid items through composition, all outcomes, exact diagnostic bytes, zero provider dispatch on precondition failure, and unchanged unrelated routes.
...
Verification intent: Exercise composition-mediated drain and close with pending and empty queues, exact diagnostics, safe repeats, stale generations, recreation, reconciliation, and cross-route isolation.
